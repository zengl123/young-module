...
import com.young.date.DateTimeUtil;
import com.young.enums.*;
import com.young.exception.UacBizException;
import com.young.manager.UserManager;
import com.young.mapper.UacActionMapper;
import com.young.mapper.UacMenuMapper;
import com.young.mapper.UacUserMapper;
import com.young.mapper.UacUserMenuMapper;
import com.young.model.Perm;
import com.young.model.SecurityUser;
import com.young.model.domain.*;
import com.young.model.dto.*;
import com.young.model.vo.MenuVo;
import com.young.model.vo.UserBindRoleVo;
import com.young.mq.producer.EmailProducer;
import com.young.security.AesUtil;
import com.young.security.Md5Util;
 ...
 * @Author:zengling
 * @钉钉:17363645521
 * @CreateDate:2019/5/15 10:43
 * @UpdateUser:
 * @UpdateDate:2019/5/15 10:43
 * @UpdateRemark:
 * @Version:
 */
@Slf4j
...
public class UacUserServiceImpl extends BaseService<UacUser> implements IUacUserService {
    @Resource
    ...
    @Resource
    private IUacGroupUserService uacGroupUserService;
    @Resource
    private IUacLogService uacLogService;
    @Resource
    private IUacRoleService uacRoleService;
    @Resource
    private IUacRoleUserService uacRoleUserService;
    @Resource
    private IUacUserMenuService uacUserMenuService;
    @Resource
    ...
    @Value("${young.auth.active-user-url}")
    private String activeUserUrl;

    @Override
    @Transactional(readOnly = true, rollbackFor = Exception.class)
    public UacUser findByLoginName(String loginName) {
        log.info("findByLoginName - 根据用户名查询用户信息. loginName={}", loginName);
    ...
    public void checkUserIsCorrect(LoginReqDto loginReqDto) {
        log.info("用户【" + loginReqDto.getLoginName() + "】进行密码认证......");
        ...
        if (PublicUtil.isEmpty(uacUser)) {
            log.info("用户【" + loginReqDto.getLoginName() + "】密码认证失败");
            throw new UacBizException(ErrorCodeEnum.UAC10011002, loginReqDto.getLoginName());
        }
        log.info("用户【" + loginReqDto.getLoginName() + "】密码认证成功");
    }
    ...
    public List<String> getUserPerms(Long userId) {
        log.info("获取用户权限列表userId={}", userId);
        //1.获取所有菜单权限
    ...
    public void bindUserRoles(BindUserRolesDto bindUserRolesDto, LoginAuthDto authResDto) {
        ...
        Long userId = bindUserRolesDto.getUserId();
        Long loginUserId = authResDto.getUserId();
        ...
        UacUser user = this.queryByUserId(userId);
        if (user == null) {
            log.error("找不到用户信息 operUserId={}", userId);
        ...
        List<UacRoleUser> userRoles = uacRoleUserService.listByUserId(userId);
        if (PublicUtil.isNotEmpty(userRoles)) {
            uacRoleUserService.deleteByUserId(userId);
        }
        // 更新用户的操作时间
    ...
    public List<UserMenuDto> queryUserMenuDtoData(LoginAuthDto authResDto) {
        ...
        Preconditions.checkArgument(!PublicUtil.isNull(authResDto, userId), "无访问权限");
        // 查询该用户下所有的菜单Id集合
    ...
    public void register(UserRegisterDto registerDto) {
        ...
        param.put("dateTime", DateTimeUtil.nowDateTimeString());
        Set<String> to = Sets.newHashSet();
        to.add(registerDto.getEmail());
        MqMessageData mqMessageData = emailProducer.sendEmailMq(to, UacEmailTemplateEnum.ACTIVE_USER, AliyunMqTopicConstant.MqTagEnum.ACTIVE_USER, param);
    ...
    public boolean checkLoginName(String loginName) {
        ...
        try {
            ...
            log.error("验证用户名是否存在,出现异常={}", e.getMessage(), e);
        }
        return result < 1;
    }
    ...
    public boolean checkEmail(String email) {
        ...
        try {
            ...
            log.error("验证email是否存在,出现异常={}", e.getMessage(), e);
        }
        return result < 1;
    }
    ...
    public boolean checkMobileNo(String mobileNo) {
        ...
        try {
            ...
            log.error("验证手机号码是否存在,出现异常={}", e.getMessage(), e);
        }
        return result < 1;
    }
    ...
    public void modifyUserEmail(String email, String emailCode, LoginAuthDto loginAuthDto) {
        // 校验用户信息
        Preconditions.checkArgument(!StringUtils.isEmpty(email), ErrorCodeEnum.UAC10011018.msg());
        Preconditions.checkArgument(PublicUtil.isEmail(email), "邮箱格式不正确");
        UacUser uacUser = new UacUser();
    ...
    public void resetLoginPwd(Long userId, LoginAuthDto loginAuthDto) {
        ...
        final MqMessageData mqMessageData = emailProducer.sendEmailMq(to, UacEmailTemplateEnum.RESET_LOGIN_PWD, AliyunMqTopicConstant.MqTagEnum.RESET_LOGIN_PWD, param);
    ...
    public void resetLoginPwd(ResetLoginPwdDto resetLoginPwdDto) {
        ...
        Preconditions.checkArgument(!StringUtils.isEmpty(confirmPwd), ErrorCodeEnum.UAC10011009.msg());
        Preconditions.checkArgument(!StringUtils.isEmpty(resetPwdKey), ErrorCodeEnum.UAC10011028.msg());
        Preconditions.checkArgument(newPassword.equals(confirmPwd), ErrorCodeEnum.UAC10011010.msg());
    ...
    public void activeUser(String activeUserToken) {
        ...
        MqMessageData mqMessageData = emailProducer.sendEmailMq(to, UacEmailTemplateEnum.ACTIVE_USER_SUCCESS, AliyunMqTopicConstant.MqTagEnum.ACTIVE_USER_SUCCESS, param);
    ...
    private void validateEmailResetPwd(ForgetResetPasswordDto forgetResetPasswordDto) {
        ...
        try {
            AesUtil.decrypt(forgetToken, forgetKey, forgetKey);
        } catch (Exception e) {
    ...
    private int deleteUserMenuList(UacUserMenu uacUserMenu) {
        ...
        if (delCount < selCount) {
            log.error("清空该用户常用菜单失败 delCount = {} selCount = {}", delCount, selCount);
    ...
    private void checkUserMenuList(List<Long> menuIdList, LoginAuthDto authResDto, List<UacUserMenu> uacUserMenuList) {
        ...
        for (Long menuId : menuIdList) {
            if (uacMenuService.checkMenuHasChildMenu(menuId)) {
                log.error(" 选择菜单不是根目录 menuId= {}", menuId);
    ...
    private void validateRegisterInfo(UserRegisterDto registerDto) {
        ...
        Preconditions.checkArgument(!StringUtils.isEmpty(mobileNo), ErrorCodeEnum.UAC10011006.msg());
        ...
        Preconditions.checkArgument(!StringUtils.isEmpty(registerDto.getRegisterSource()), ErrorCodeEnum.UAC10011017.msg());
        Preconditions.checkArgument(registerDto.getLoginPwd().equals(registerDto.getConfirmPwd()), ErrorCodeEnum.UAC10011010.msg());
        UacUser uacUser = new UacUser();
        uacUser.setLoginName(registerDto.getLoginName());
